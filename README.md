...
For monthly report scheduling on Netlify, hit `POST /api/cron/monthly-reports` hourly at :30 from 15:30-23:30 UTC on the first 3 days of each month with the cron secret. The app will stagger enabled websites in deterministic 10-minute slots starting at 11:30 AM `America/New_York` on the 1st of the month, and it will catch up missed slots on the next run.
