...
For monthly report scheduling on Netlify, hit `POST /api/cron/monthly-reports` hourly at :40 from 15:40-23:40 UTC on the first 3 days of each month with the cron secret. The app will stagger enabled websites in deterministic 10-minute slots starting at 11:40 AM `America/New_York` on the 1st of the month, and it will catch up missed slots on the next run.
